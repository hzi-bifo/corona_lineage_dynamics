...
Rscript scripts/get_alias_list.R $alias $lineage_dates
...
scripts/count_lineages_country $gisaid_file $time $output_folder "false" $alias $lineage_dates
...
scripts/count_lineages_country $gisaid_file $time $output_folder "true" $alias $lineage_dates

echo "STARTING ./SDPlots_lineages.sh - 4"
date
# count lineages for German states
Rscript scripts/count_lineages_states.R $gisaid_file $time $output_folder "false" $alias $lineage_dates
Rscript scripts/count_lineages_states.R $gisaid_file $time $output_folder "true" $alias $lineage_dates

echo "STARTING ./SDPlots_lineages.sh - 5"
date
# in addition to monthly analysis: sliding window fisher test
Rscript scripts/fishertest_continuous.R $gisaid_file $threshold $output_folder
...
for folder in $output_folder/*/ ; do
	...
	pattern=" |'"
	if [[ $string =~ $pattern ]]; then continue; fi
	...
	sbatch -o $output_folder/control/$country.out -e $output_folder/control/$country.err -q long -t 10-00:00:00 --mem-per-cpu 10g --cpus-per-task 4 SDPlots_country.sh $country $folder $threshold
	if [ "$?" != "0" ]
	then
		echo "sbatch failed"
		exit 1
...
Rscript scripts/summary_statistics.R $output_folder $time 0.1
